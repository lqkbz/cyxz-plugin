...
/** 从API列表中随机获取图片
 * @param {Array} api_urls - API URL列表
 * @param {int} user_id - 发送消息的用户ID
 * @returns {Promise<Array>} 返回聊天记录数组
 */
async function getRandomApiImage(api_urls, user_id = 8000000) {
    try {
        // 随机选择一个API
        const randomUrl = api_urls[Math.floor(Math.random() * api_urls.length)];
        ...
        } else if (typeof response.data === 'object' && response.data.data && response.data.data.url) {
            // 返回嵌套JSON的API
        ...
        msgList.push({
            user_id: user_id,
            nickname: '图片',
            message: await segment.image(imageUrl)
        });

        return msgList;
        
    } catch (error) {
        logger.error('获取API图片时发生错误:', error.message);
        return [{
            user_id: user_id,
            nickname: '错误提示',
...
export class dimtown extends plugin {
    ...
    async sex(e) {
        // API列表 - 从各种图片API中随机选择
        const api_urls = [
            'https://www.acy.moe/api/r18',
            'https://moe.jitsu.top/api/?sort=r18&size=small&type=302'
        ];
        
        try {
            e.reply("雑魚~♡雜魚~♡");
            const forwardMsg = await Bot.makeForwardMsg(await getRandomApiImage(api_urls, e.user_id));
            e.reply(forwardMsg);
        } catch (error) {
            logger.error('发送色图时发生错误:', error.message);
            e.reply('获取图片失败，请稍后重试');
        }
        return true;
    }
